    ...
    void Move()
    {
        ...
        switch (L)
        {
            ...
            case Level.Normal:   
                ...
                {
                    if(GameManagement.instance.mode==Mode.ColorWar)
                    {
            ...
            case Level.Hard:
                foreach (Collider2D c in colliders)
                {
                    ...
                    else
                    {
                        ...
                        if (c.transform.localScale.x > R)//逃跑
                        {
                            runDir = c.transform.position - transform.position;
                            runLen = Mathf.Min(catchLen, catchDir.magnitude);
                        }
                    }
                }
